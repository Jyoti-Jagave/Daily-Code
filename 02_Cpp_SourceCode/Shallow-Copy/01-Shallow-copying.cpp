...
class IntPtr
{
    ...
    private:
          ...
          IntPtr(int_m) : p(new int (_m)), reference_count?(1) {}
          ~IntPtr() {delete p;}
...
class HasPtr
{
    ...
    public:
        ...
        ~HasPtr()
        {
            std::cout << "In HasPtr::~HasPr()" << std::endl;
            if(--pIntPtr->reference_count == 0)
            {
                std::cout << "Deleting shreed pIntPtr" << std::endl;
                delete pIntPtr;
            }
        }
...
void test()
{
    ...
    std::cout << "phpl->getrefcout():" << php1->getrefcount() << std::endl;
    HasPtr* php2 = new HasPtr(*php1);
    std::cout << "phpl->getrefcount():" << php1->getrefcount() << std::endl;
